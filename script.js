...
for (let i = 0; i < 15; i++) {
  const hue = 300 + Math.random() * 60; // pink to rose hues
  const color = `hsla(${hue}, 90%, 55%, 0.5)`;
...
const galaxyParameters = {
  ...
  outsideColor: new THREE.Color(0xff9eb5),
};
...
for (let i = 0; i < galaxyParameters.count; i++) {
  ...
  const mixedColor = new THREE.Color(0xff1a4d);
  mixedColor.lerp(new THREE.Color(0xffb3c6), radius / galaxyParameters.radius);
...
function createShootingStar() {
  ...
  const atmosphereMaterial = new THREE.ShaderMaterial({
    uniforms: {
      glowColor: { value: new THREE.Color(0xff9eb5) }
    },
    vertexShader: `
        varying vec3 vNormal;
        void main() {
...
const ringTexts = [
  'With all my heart ...',//untuk deskripsi planet layer 1
  "I love you ♡",//untuk deskripsi planet layer 2
  "Forever and always ♡",//untuk deskripsi planet layer 3
  "01/08/2025",//untuk deskripsi planet layer 4
  ...(window.dataCCD && window.dataCCD.data.ringTexts ? window.dataCCD.data.ringTexts : [])
];

// --- Teks di dalam bola planet ---
function createPlanetText() {
  ...
  const texts = ringTexts.slice(0, 4);
  const lineHeight = 90;
  const totalH = (texts.length - 1) * lineHeight;
  ...
  texts.forEach((text, i) => {
    ...
    ctx.fillText(text, size / 2, startY + i * lineHeight);
  });
  ...
  const geometry = new THREE.PlaneGeometry(18, 14);
  const mesh = new THREE.Mesh(geometry, material);
  mesh.name = 'planet-text';
  scene.add(mesh);
  return mesh;
}
const planetText = createPlanetText();
...
function animate() {
  ...
  if (planetText) planetText.lookAt(camera.position);
  animatePlanetSystem();
...
function createHintText() {
  ...
  context.shadowColor = '#ff6699';
  context.shadowBlur = 5;
  context.lineWidth = 2;
  context.strokeStyle = 'rgba(255, 100, 150, 0.8)';
  context.strokeText(text, canvasSize / 2, canvasSize / 2);
  context.shadowColor = '#ff3366';
  context.shadowBlur = 5;
  context.lineWidth = 2;
  context.strokeStyle = 'rgba(255, 50, 100, 0.5)';
  context.strokeText(text, canvasSize / 2, canvasSize / 2);
